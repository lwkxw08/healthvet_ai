...
def generate_candidate_audit_docx(candidate_id: str) -> bytes:
    ...
    score_heading = doc.add_paragraph()
    score_heading.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sr = score_heading.add_run(f"Compliance Score: {score:.0f}% ({risk_level} RISK)")
    sr.bold = True
    sr.font.size = Pt(14)
    sr.font.color.rgb = BRAND_NAVY

    # Compliance summary table
    summary_headers = ["Section", "Status"]
    summary_rows = [
    ...
    _add_styled_table(doc, summary_headers, summary_rows, col_widths=[8, 8])
    ...
    _add_kv_table(doc, [
        ...
        ("Name", f"{_s(c.get('first_name'), '')} {_s(c.get('last_name'), '')}"),
        ("Email", _s(c.get("email"))),
        ("Role", _s(c.get("profession"), "Healthcare Professional")),
        ("Compliance Status", compliance_status),
        ("CQC Ready", cqc_ready),
        ("Risk Level", risk_level),
        ("Report Generated", now.strftime("%d %B %Y at %H:%M UTC")),
    ])
    ...
    summary_text = (
        "Candidate is fully compliant with all required checks and suitable for deployment"
